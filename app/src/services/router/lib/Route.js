...
Route.prototype.raw = function(v) {
  return this._raw;
};
Route.prototype.setRaw = function(v) {
  this._raw = v;
  return this;
};
Route.prototype.params = function(v) {
  return this._params;
};
Route.prototype.setParams = function(v) {
  this._params = v;
  return this;
};
Route.prototype.__status = { watch: 'statusDidSet', name: 'status' };
Route.prototype.status = function(v) {
  return this._status;
};
Route.prototype.setStatus = function(v) {
  ...
  if (v != a) {
    this.statusDidSet && this.statusDidSet(v, a, this.__status);
  }
  return this;
};
...
Route.prototype.test = function(loc, path) {
  ...
  if (url == this._cache.url) {
    return this._cache.match;
  }
  ...
  if (this._query) {
    qmatch = {};
    for (
      let o = this._query, v, i = 0, keys = Object.keys(o), l = keys.length, k;
      i < l;
      i++
    ) {
      k = keys[i];
  ...
  if ((match = this._regex ? matcher.match(this._regex) : [''])) {
    ...
    if (fullpath == this._params.path) {
      ...
      if (this._groups.length) {
        for (
          let i = 0, items = iter$(match), len = items.length, item, name;
          i < len;
          i++
        ) {
          item = items[i];
    ...
    if (qmatch) {
      for (
        let v, i = 0, keys = Object.keys(qmatch), l = keys.length, k;
        i < l;
        i++
      ) {
        k = keys[i];
...
Route.prototype.statusDidSet = function(status, prev) {
  ...
  return (
    this._node &&
    this._node.setFlag &&
    this._node.setFlag('route-status', 'status-' + status)
  );
};
...
Route.prototype.load = function(cb) {
  ...
  var handler = (self._handler = function(res) {
    ...
    return (
      self.setStatus(
        (v_ = typeof res == 'number' || res instanceof Number ? res : 200)
      ),
      v_
    );
  });
